...
$dbc->query("DELETE FROM staffs WHERE user_id={$_REQUEST['delete']}");
...
$sid = mysqli_real_escape_string($dbc,trim($_POST['s_id']));
$userid = mysqli_real_escape_string($dbc,trim($_POST['user_id']));
...
$add2 = $dbc->query($query2);
        ...
        if($add && $add2){
            $message = "Staff ID: ". $sid . " edited successfully.";
        }else{
            echo $query . '<br>';
echo $query1 . '<br>';
echo $query2 . '<br>';
...
<div style="
    ...
    margin-top: 10px;
    font-size: 15px;
"><?php if ($_SERVER['REQUEST_METHOD']=='POST'){ if($errors) {echo '<span style="color: red;">Unable to save edited staff:</span><br><br><span>'.implode("<br>",$errors) . '</span>';}else { echo '<span style="color:red;">'.$message .'</span>';} } ?></div>
...
<form action="edit_staff.php" method="post" class="basic-grey">
...
"><span>--------------Next of Kin--------------</span></div>
   <label>
        <span>Name :</span>
        <input type="text" name="nextname" placeholder="Full name"id="nextname">
    </label> 
    <label>
        <span>Address :</span>
        <input type="text" name="nextaddress" placeholder="Valid Address" id="nextaddress">
    </label>  
     <label>
   ...
   <div class="modal-footer" style="margin-bottom: -26px;">
     <label>
        <span>&nbsp;</span> 
