 ...
 if (window.XMLHttpRequest) {
  ...
  xmlhttp.onreadystatechange=function() {
    if (xmlhttp.readyState==4 && xmlhttp.status==200) {
      document.getElementById("tblFuel").innerHTML=xmlhttp.responseText;
    }
  }
  xmlhttp.open("GET","get_fuel_reserved.php",true);
  xmlhttp.send();
...
$("#fuel_allocated").text( $(event.target).closest('a').data('fuel_allocated'));
$("#date").text( $(event.target).closest('a').data('date'));
$("#fuel_reserve").text( $(event.target).closest('a').data('fuel_reserve'));
    });
});
...
<tr>
	<th>Fuel Allocated</th><td id="fuel_allocated"></td>
</tr>
<tr>
	<th>Fuel Reserved</th><td id="fuel_reserve"></td>
</tr>
<tr>
	<th>Time</th><td id="date"></td>
</tr>
