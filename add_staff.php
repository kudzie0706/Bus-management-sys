<?php $title = 'Add Staff'; // In this line I am defining the Title of the page which works with the design of the header.html?>
...
<?php
    ...
    $errors = array();
    if(empty($_POST['name'])){
        $errors['name'] = 'Please fill in staff name';//name
  ...
  $stripped_next = preg_replace('/[\(\)\.\-\ ]/', '', $_POST['nextphone']);
    ...
    if(empty($errors)){
        ...
        $query .= "VALUES ('$name','$address','$phone','$sex','$email','$password','0')";
...
do {  
  $staff_id = '';
  for ($i = 0; $i < 6; $i++) {
    $staff_id .= rand(0, 1) ? rand(0, 9) : chr(rand(ord('A'), ord('Z')));
  }
 if($check = $dbc->query("SELECT s_id FROM staffs WHERE s_id = '$staff_id'")){
            if($check->num_rows){
                $exist = true;
            }
            else
...
$query1 =  "INSERT INTO staffs (s_id,user_id,salary,hireddate) VALUES ('$staff_id','$user_id','$salary','$hireddate')";

$insertstaff = $dbc->query($query1);

$query2 =  "INSERT INTO nextofkin (user_id,name,address,sex,phone) VALUES ('$user_id','$nextname','$nextaddress','$nextsex','$nextphone')";

$insertnext = $dbc->query($query2);

$nid = $dbc->query("SELECT nid FROM nextofkin WHERE user_id = '$user_id'")->fetch_object()->nid;


$query3 = "UPDATE users SET nid='$nid' WHERE user_id = '$user_id'";

$updatenid = $dbc->query($query3);

        if($register && $insertnext && $updatenid && $insertstaff){
            $message = $name . " has been added to staff.";
        }else{
            echo $query . '<br>';
echo $query2 . '<br>';
        }
...
<div id="content">
<form action="add_staff.php" method="post" class="basic-grey">
    <h1>Add Staff<?php if ($_SERVER['REQUEST_METHOD']=='POST'){ if(empty($errors)){ echo '<span style="color:red;">'.$message; }else { echo '<span style="color:red">Opps.. something wrong!</span><span>'.implode("<br>",$errors);}} else { echo '<span>Please fill all the texts in the fields.'; } ?></span>
    </h1>
<div style="
    text-align: center;
    margin-bottom: 10px;
margin-top: -16px;
"><span">--------------Staff Information--------------</span></div>
    <label>
